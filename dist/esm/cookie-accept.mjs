...
import ConsentCookie from './ConsentCookie.mjs';
import ConsentCheckboxes from './ConsentCheckboxes.mjs';
...
var CookieAccept = /*#__PURE__*/function () {
  ...
  return _createClass(CookieAccept, [{
    ...
    value: function _init() {
      var _this = this;
      var cookieValue = ConsentCookie.getValue(this.cookieName);
      if (cookieValue) {
      ...
      this.acceptTriggers.forEach(function (trigger) {
        trigger.addEventListener('click', function () {
          var newCookieValue = ConsentCheckboxes.toCookieValuesForcedToTrue(_this.checkboxes);
          _this._updateCookie(newCookieValue);
          _this.checkboxes = ConsentCheckboxes.updateCheckboxesByCookieValue(_this.checkboxes, newCookieValue);
        });
      });
      this.rejectTriggers.forEach(function (trigger) {
        trigger.addEventListener('click', function () {
          var newCookieValue = ConsentCheckboxes.toCookieValuesForcedToFalse(_this.checkboxes, Object.keys(_this.consents).filter(function (consentName) {
            return !!_this.consents[consentName];
          }));
          _this._updateCookie(newCookieValue);
          ConsentCheckboxes.updateCheckboxesByCookieValue(_this.checkboxes, newCookieValue);
        });
      });
      this.updateTriggers.forEach(function (trigger) {
        trigger.addEventListener('click', function () {
          var newCookieValue = ConsentCheckboxes.toCookieValues(_this.checkboxes);
          _this._updateCookie(newCookieValue);
        });
      });
    }
...
export { CookieAccept as default };